...
const totalPresupuesto = computed(() => {
  ...
  Object.values(elementosEstanciaOrganizados).forEach((categoria) => {
    ...
    categoria.forEach((elemento) => {
      if (elementosSeleccionados.value[elemento.concepto]) {
        let cantidad;
        if (elemento.unidad.includes("m2") && superficie.value) {
          // Si la unidad es m2 y hay valor en 'superficie'
          cantidad = parseFloat(superficie.value);
        } else if (elemento.unidad.includes("ml") && longitud.value) {
          // Asumimos que tienes una variable 'longitud' para metros lineales
          // Si la unidad es ml y hay valor en 'longitud'
          cantidad = parseFloat(longitud.value);
        } else {
          // Para cualquier otra unidad o si los valores específicos no están disponibles
